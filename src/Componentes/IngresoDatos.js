...
function IngresoDatos() {
    ...
    const [mensajeError1, setMensajeError1] = useState(false);
    const [mensajeError2, setMensajeError2] = useState(false);
    const [mensajeError3, setMensajeError3] = useState(false);
    const [mensajeError4, setMensajeError4] = useState(false);
    const [mensajeError5, setMensajeError5] = useState(false);
    const [mensajeError6, setMensajeError6] = useState(false);
    ...
    const [isFixedMO, setIsFixedMO] = useState(false);
    const [ingresoLecheMO, setIngresoLecheMO] = useState('');
    ...
    const [isFixedOGD, setIsFixedOGD] = useState(false);
    const [ingresoLecheOGD, setIngresoLecheOGD] = useState('');
    ...
    const [isFixedGE, setIsFixedGE] = useState(false);
    const [ingresoLecheGE, setIngresoLecheGE] = useState('');
    ...
    const mortandadVacas = (parseFloat(vacasMuertas) / parseFloat(vacasTotales) * 100).toFixed(1);
    ...
    let ingresoLecheManoObra;
    if (!isFixedMO) {
        ingresoLecheManoObra = ingresoLeche;
    } else {
        ingresoLecheManoObra = ingresoLecheMO;
    }
    const gastoManoDeObra = (parseFloat(gastoManoDeObraP) / 100 * parseFloat(ingresoLecheManoObra)).toFixed(0);
    const gastoPorOperario = (parseFloat(gastoManoDeObra) / parseFloat(cantidadOperarios)).toFixed(0);

    //Cálculos sección 4

    const cantidadVqReposicion = (parseFloat(cantidadVaquillonas) - parseFloat(vaquillonasCab)).toFixed(0);
    const porcentajeReposicion = (parseFloat(cantidadVqReposicion) / parseFloat(vacasTotales) * 100).toFixed(1);
    ...
    const bajaVacas = (parseFloat(vacasCab) + parseFloat(vacasMuertas)).toFixed(0);
    const bajaVacasP = (parseFloat(bajaVacas) / parseFloat(vacasTotales) * 100).toFixed(1);
    const variacionVacas = (parseFloat(cantidadVqReposicion) - parseFloat(bajaVacas)).toFixed(0);

    //Cálculos sección 5
    let ingresoLecheOtrosGastosDirectos;
    if (!isFixedOGD) {
        ingresoLecheOtrosGastosDirectos = ingresoLeche;
    } else {
        ingresoLecheOtrosGastosDirectos = ingresoLecheOGD;
    }
    const gastoAlimentacion = (parseFloat(gastoAlimentacionP) * parseFloat(ingresoLecheOtrosGastosDirectos) / 100).toFixed(0);
    const gastoSuministro = (parseFloat(gastoSuministroP) * parseFloat(ingresoLecheOtrosGastosDirectos) / 100).toFixed(0);
    const gastosVeterinaria = (parseFloat(gastosVeterinariaP) * parseFloat(ingresoLecheOtrosGastosDirectos) / 100).toFixed(0);
    const gastosRodeo = (parseFloat(gastosRodeoP) * parseFloat(ingresoLecheOtrosGastosDirectos) / 100).toFixed(0);
    const alquilerVacas = (parseFloat(alquilerVacasP) * parseFloat(ingresoLecheOtrosGastosDirectos) / 100).toFixed(0);
    const gastosTambo = (parseFloat(gastosTamboP) * parseFloat(ingresoLecheOtrosGastosDirectos) / 100).toFixed(0);

    //Cálculos sección 6
    let ingresoLecheGastosEstructura;
    if (!isFixedGE) {
        ingresoLecheGastosEstructura = ingresoLeche;
    } else {
        ingresoLecheGastosEstructura = ingresoLecheGE;
    }
    const gastosMantenimiento = (parseFloat(gastosMantenimientoP) * parseFloat(ingresoLecheGastosEstructura) / 100).toFixed(0);
    const gastoArrendamiento = (parseFloat(gastoArrendamientoP) * parseFloat(ingresoLecheGastosEstructura) / 100).toFixed(0);
    const impuestos = (parseFloat(impuestosP) * parseFloat(ingresoLecheGastosEstructura) / 100).toFixed(0);
    const gerencia = (parseFloat(gerenciaP) * parseFloat(ingresoLecheGastosEstructura) / 100).toFixed(0);
    const gastosAdministracion = (parseFloat(gastosAdministracionP) * parseFloat(ingresoLecheGastosEstructura) / 100).toFixed(0);
    ...
    const diferenciaInventario = (parseFloat(variacionVacas) * parseFloat(costoVaquillona) * parseFloat(precioLeche)).toFixed(0);
    const diferenciaInventarioLitro = (parseFloat(diferenciaInventario) / parseFloat(lecheVendida)).toFixed(3);
    const ingresoBruto2 = (parseFloat(ingresoBruto) + parseFloat(diferenciaInventario)).toFixed(0);
    const resultadoOperativo = (parseFloat(ingresoBruto2) - parseFloat(gastosDirectos) - parseFloat(gastosEstructura)).toFixed(0);
    const costoLitroCP = ((parseFloat(gastosDirectos) + parseFloat(gastosEstructura) - parseFloat(ingresoCarne) - parseFloat(ingresoVq) - parseFloat(diferenciaInventario)) / parseFloat(lecheVendida)).toFixed(3);
    ...
    const resultadoOpP = (parseFloat(resultadoOperativo) / parseFloat(ingresoBruto2) * 100).toFixed(1);
    const gastosDirectosP = (parseFloat(gastosDirectos) / parseFloat(ingresoBruto2) * 100).toFixed(1);
    const gastosEstructuraP = (parseFloat(gastosEstructura) / parseFloat(ingresoBruto2) * 100).toFixed(1);
    ...
    const recuperoCarneLitro = ((parseFloat(ingresoCarne) + parseFloat(ingresoVq) + parseFloat(diferenciaInventario)) / parseFloat(lecheVendida)).toFixed(3);
    ...
    if (!formatoEnteroPositivo.test(cantidadVaquillonas) || !formatoEnteroPositivo.test(costoVaquillona) ||
        (parseFloat(vaquillonasCab) > parseFloat(cantidadVaquillonas))) {
        validacion4 = false;
    }
    ...
    const handleVacasMuertasChange = (e) => {
        setVacasMuertas(e.target.value);
    };
    ...
    useEffect(() => {
        if (cambiarMoneda === true) {
            ...
            setIngresoLecheMO((parseFloat(ingresoLecheMO) * parseFloat(tipoCambio)).toFixed(3));
            setIngresoLecheOGD((parseFloat(ingresoLecheOGD) * parseFloat(tipoCambio)).toFixed(3));
            setIngresoLecheGE((parseFloat(ingresoLecheGE) * parseFloat(tipoCambio)).toFixed(3));
        }
        if (cambiarMoneda === false) {
            setPrecioLeche((parseFloat(precioLeche) / parseFloat(tipoCambio)).toFixed(3));
            setPrecioVacas((parseFloat(precioVacas) / parseFloat(tipoCambio)).toFixed(3));
    ...
    const handleCheckboxChangeMO = (e) => {
        setIsFixedMO(e.target.checked);
        if (!isFixedMO) {
            setIngresoLecheMO(ingresoLeche);
        }
    };
    ...
    const handleCheckboxChangeOGD = (e) => {
        setIsFixedOGD(e.target.checked);
        if (!isFixedOGD) {
            setIngresoLecheOGD(ingresoLeche);
        }
    };
    ...
    const handleCheckboxChangeGE = (e) => {
        setIsFixedGE(e.target.checked);
        if (!isFixedGE) {
            setIngresoLecheGE(ingresoLeche);
        }
    };

    // Respuesta a botones de calcular

    const handleClick1 = () => {
        if (!validacion1) {
            setMensajeError1(true);
        } else {
            setForzador(prevstate => !prevstate);
            setMostrarSeccion2(true);
            setMensajeError1(false);
        }
    };

    const handleClick2 = () => {
        if (!validacion2) {
            setMensajeError2(true);
        } else {
            setMostrarSeccion3(true);
            setMensajeError2(false);
        }
    };

    const handleClick3 = () => {
        if (!validacion3) {
            setMensajeError3(true);
        } else {
            setMostrarSeccion4(true);
            setMensajeError3(false);
        }
    };

    const handleClick4 = () => {
        if (!validacion4) {
            setMensajeError4(true);
        } else {
            setMostrarSeccion5(true);
            setMensajeError4(false);
        }
    };

    const handleClick5 = () => {
        if (!validacion5) {
            setMensajeError5(true);
        } else {
            setMostrarSeccion6(true);
            setMensajeError5(false);
        }
    };

    const handleClick6 = () => {
        if (!validacion6) {
            setMensajeError6(true);
        } else {
            setMostrarSeccion7(true);
            setMensajeError6(false);
        }
    };
    ...
    return (
        <div>
            ...
            <div className='seccion'>
                ...
                <form>
                    ...
                    <div className='seccionFormulario'>
                        ...
                        {validacion3 && (<div>
                            <h4 style={{ color: "darkorange", backgroundColor: "lightyellow" }} id='advertencia'>ADVERTENCIA: Si cambia los litros, revise los porcentajes de los rubros de gastos o fije sus valores en dinero</h4>
                            <Tooltip anchorSelect="#advertencia" place="top" className='tooltip'>
                                <p><b>ADVERTENCIA:</b></p>
                                <p>Algunos rubros de gastos se ingresaron como</p>
                                ...
                                <p>cambiarán en la misma proporción dichos gastos.</p>
                                <p>Revise todos los gastos si decide cambiar los litros.</p>
                                <p>También puede utilizar las opciones de fijar grupos</p>
                                <p>de gastos en dinero (mano de obra, gastos directos y</p>
                                <p>gastos de estructura) según corresponda.</p>
                            </Tooltip>
                        </div>)}
                    ...
                    <div className='seccionFormulario'>
                        <div>{(ventaVacas || ventaToros || ventaToritos || ventaTerneros || ventaTerneras
                            || ventaVaquillonas) && (<div>
                                ...
                                <Tooltip anchorSelect="#datosVentaCarne" place="top" className='tooltip'>
                                    ...
                                    <p>cantidad de animales exportados por</p>
                                    <p>año y peso promedio en kilogramos</p>
                        ...
                        <div className='seccionFormulario'>
                            <label id="vacasMuertas">Vacas muertas (cantidad/año): </label>
                            <input type='number' value={vacasMuertas} onChange={handleVacasMuertasChange} placeholder='Ingresar cantidad de vacas muertas por año' />
                            <Tooltip anchorSelect="#vacasMuertas" place="top" className='tooltip'>
                                <p><b>Vacas muertas:</b></p>
                                <p>Número promedio anual de vacas muertas</p>
                                <p>- No ingresar decimales - </p>
                            </Tooltip>
                        </div>
                    </div>
            ...
            {mostrarSeccion2 && (<div className='seccion'>
                <h3>Ingresos brutos por ventas:</h3>
                <p>(paso 2 de 6)</p>
                <div className='seccionFormulario'>
                ...
                <form>
                    <div className='seccionFormulario'>
                        ...
                        <a href={`https://www.xe.com/currencyconverter/convert/?Amount=1&From=${codigoMoneda2}&To=${codigoMoneda1}`} target="_blank" rel="noopener noreferrer">Consultar conversor de monedas</a>
                    </div>
                    <div className='seccionFormulario'>
                    ...
                    {validacion3 && (<div>
                        <h4 style={{ color: "darkorange", backgroundColor: "lightyellow" }} id="advertencia2">ADVERTENCIA: Si cambia el precio, revise los porcentajes de los rubros de gastos o fije sus valores en dinero</h4>
                    ...
                    {ventaVaquillonas && (<div className='seccionFormulario'>
                        ...
                        <input type='number' value={precioVqLitros} onChange={handlePrecioVqLitrosChange} placeholder='Ingresar precio en litros/cabeza' />
            ...
            {mostrarSeccion3 && (<div className='seccion'>
                ...
                <p>(paso 3 de 6)</p>
                <form>
                    {!isFixedMO && (<div className='seccionFormulario'>
                ...
                {mostrarSeccion4 && (<div>
                    <ManoDeObra validacion3={validacion3} gastoManoDeObra={gastoManoDeObra}
                        gastoPorOperario={gastoPorOperario} codigoMoneda={codigoMoneda} ingresoLeche={ingresoLeche} />
                    <div id="fijarMO">
                        <div className='check2'>
                            <label id='fijar'><b>MO</b> en {codigoMoneda}</label>
                            <input
                                className='checkbox'
                                type="checkbox"
                                checked={isFixedMO}
                                onChange={handleCheckboxChangeMO} />
                            <Tooltip anchorSelect="#fijar" place="top" className='tooltip'>
                                <p><b>Fijar MANO DE OBRA en dinero:</b></p>
                                <p>Dado que los valores de esta sección se ingresan</p>
                                <p>como porcentuales del ingreso en leche, esta opción</p>
                                <p>sirve para fijarlos en dinero, ante cambios en el precio</p>
                                <p>de la leche o en los litros de leche vendidos.</p>
                            </Tooltip>
                        </div>
                    </div>
                </div>
            ...
            {mostrarSeccion4 && (<div className='seccion'>
                <h3>Reposición y variación de rodeo:</h3>
                <p>(paso 4 de 6)</p>
                <form>
                    <div className='seccionFormulario'>
                        <label id="precioVaquillona">Costo vaquillona reposición (litros leche):</label>
                        <label style={{ fontSize: "small" }}>Costo propio o de compra (o combinando ambos)</label>
                        ...
                        <Tooltip anchorSelect="#precioVaquillona" place="top" className='tooltip'>
                            <p><b>Costo de la vaquillona de reposición:</b></p>
                        ...
                        {(precioVqLitros > 0 && kilosVaquillonas > 0) && (<div className='checklist'>
                            <div className='opciones'><p>Valor de venta declarado (paso 2): {new Intl.NumberFormat().format(precioVqLitros)} litros/cab</p></div>
                        </div>)}
                    </div>
                    <div className='seccionFormulario'>
                        <label id="cantidadVaquillonas">Vaquillonas disponibles por año (cantidad):</label>
                        <input type='number' value={cantidadVaquillonas} onChange={handleCantidadVaquillonasChange} placeholder='Vaquillonas para reposición y/o venta' />
                        <Tooltip anchorSelect="#cantidadVaquillonas" place="top" className='tooltip'>
                            <p><b>Vaquillonas disponibles por año:</b></p>
                            <p>Cantidad total de vaquillonas que están disponibles para:</p>
                            <p><b>* Reposición propia</b></p>
                            <p><b>* Venta</b></p>
                            <p>Se incluyen vaquillonas compradas, en caso de corresponder</p>
                            <p><b>ADVERTENCIA: El número de vaquillonas disponibles</b></p>
                            <p><b>no puede ser menor que el número de vaquillonas vendidas</b></p>
                            <p>- No ingresar decimales -</p>
                        </Tooltip>
                    </div>
                </form>
                ...
                {mostrarSeccion5 && (<div>
                    <Reposicion validacion4={validacion4} porcentajeReposicion={porcentajeReposicion}
                        ...
                        codigoMoneda={codigoMoneda} cantidadVqReposicion={cantidadVqReposicion}
                        vaquillonasCab={vaquillonasCab} cantidadVaquillonas={cantidadVaquillonas}
                        bajaVacas={bajaVacas} bajaVacasP={bajaVacasP} vacasCab={vacasCab} vacasMuertas={vacasMuertas}
                    />
                </div>
                )}
            </div>
            ...
            {mostrarSeccion5 && (<div className='seccion'>
                ...
                {mostrarSeccion6 && (<div>
                    <OtrosGastosDirectos validacion5={validacion5} gastoAlimentacion={gastoAlimentacion}
                        gastoSuministro={gastoSuministro} gastosVeterinaria={gastosVeterinaria}
                        gastosRodeo={gastosRodeo} alquilerVacas={alquilerVacas} gastosTambo={gastosTambo}
                        codigoMoneda={codigoMoneda} ingresoLeche={ingresoLeche} />
                    <div id="fijarOGD">
                        <div className='check2'>
                            <label id='fijar1'><b>GD</b> en {codigoMoneda}</label>
                            <input
                                className='checkbox'
                                type="checkbox"
                                checked={isFixedOGD}
                                onChange={handleCheckboxChangeOGD} />
                            <Tooltip anchorSelect="#fijar1" place="top" className='tooltip'>
                                <p><b>Fijar OTROS GASTOS DIRECTOS en dinero:</b></p>
                                <p>Dado que los valores de esta sección se ingresan</p>
                                <p>como porcentuales del ingreso en leche, esta opción</p>
                                <p>sirve para fijarlos en dinero, ante cambios en el precio</p>
                                <p>de la leche o en los litros de leche vendidos.</p>
            ...
            {mostrarSeccion6 && (<div className='seccion'>
                ...
                {mostrarSeccion7 && (<div>
                    <GastosEstructura validacion6={validacion6} gastosMantenimiento={gastosMantenimiento}
                        gastoArrendamiento={gastoArrendamiento}
                        impuestos={impuestos} gerencia={gerencia} gastosAdministracion={gastosAdministracion}
                        codigoMoneda={codigoMoneda} ingresoLeche={ingresoLeche} />
                    <div id="fijarGE">
                        <div className='check2'>
                            <label id='fijar2'><b>GE</b> en {codigoMoneda}</label>
                            <input
                                className='checkbox'
                                type="checkbox"
                                checked={isFixedGE}
                                onChange={handleCheckboxChangeGE} />
                            <Tooltip anchorSelect="#fijar2" place="top" className='tooltip'>
                                <p><b>Fijar GASTOS DE ESTRUCTURA en dinero:</b></p>
                                <p>Dado que los valores de esta sección se ingresan</p>
                                <p>como porcentuales del ingreso en leche, esta opción</p>
                                <p>sirve para fijarlos en dinero, ante cambios en el precio</p>
                                <p>de la leche o en los litros de leche vendidos.</p>
            ...
            {mostrarSeccion7 && (<div className='seccion2'>
                ...
                <Tooltip anchorSelect="#resultadosEconomicos" place="top" className='tooltip'>
                    ...
                    <p>más/menos la variación del rodeo de vacas adultas.</p>
                ...
                <ResultadosEconomicos validacion8={validacion8} codigoMoneda={codigoMoneda} gastosDirectos={gastosDirectos}
                    gastosEstructura={gastosEstructura} precioLeche={precioLeche} costoLitroCP={costoLitroCP}
                    ingresoLeche={ingresoLeche} ingresoCarne={ingresoCarne} ingresoVq={ingresoVq} lecheVendida={lecheVendida}
                    ...
                    resultadoOpLecheVT={resultadoOpLecheVT} ingresoBruto2={ingresoBruto2} gastosDirectosP={gastosDirectosP}
                    gastosEstructuraP={gastosEstructuraP} gastosDirectosLitro={gastosDirectosLitro} gastosEstructuraLitro={gastosEstructuraLitro}
                    recuperoCarneLitro={recuperoCarneLitro} diferenciaInventario={diferenciaInventario} diferenciaInventarioLitro={diferenciaInventarioLitro}
                />
                {validacion7 && validacion8 && (<div>
                    <GraficoIBGDGE resultadoOperativo={resultadoOperativo} gastosDirectos={gastosDirectos} gastosEstructura={gastosEstructura}
                        ingresoBruto2={ingresoBruto2} codigoMoneda={codigoMoneda} validacion6={validacion6} graficosEnMoneda={graficosEnMoneda}
                    />
                    <GraficoAplicacionIB validacion6={validacion6} codigoMoneda={codigoMoneda} resultadoOperativo={resultadoOperativo}
                        ...
                        impuestos={impuestos} gerencia={gerencia} gastosAdministracion={gastosAdministracion}
                        graficosEnMoneda={graficosEnMoneda} ingresoBruto2={ingresoBruto2}
                    />
                ...
                {validacion8 && (<div className='resultados'>
                    <PDFDownloadLink document={<ReportePDF nombreCaso={nombreCaso} vacasOrdeno={vacasOrdeno} vacasSecas={vacasSecas}
                        ...
                        kilosCarne={kilosCarne} productividadCarne={productividadCarne} gastosDirectosLitro={gastosDirectosLitro} costoVaquillona={costoVaquillona}
                        ...
                        kilosVaquillonas={kilosVaquillonas} cabezasVendidas={cabezasVendidas} diferenciaInventario={diferenciaInventario}
                        diferenciaInventarioLitro={diferenciaInventarioLitro} vacasMuertas={vacasMuertas} mortandadVacas={mortandadVacas}
                        cantidadVqReposicion={cantidadVqReposicion} bajaVacasP={bajaVacasP} precioVacas={precioVacas} precioToros={precioToros}
                        precioToritos={precioToritos} precioTerneros={precioTerneros} precioTerneras={precioTerneras} precioVqLitros={precioVqLitros}
